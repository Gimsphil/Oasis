...
from PyQt6.QtWidgets import (
    ...
    QListWidgetItem, QFrame, QSplitter, QDialog, QTableWidget,
...
class LightingPowerPopup(QDialog):
    ...
    def __init__(self, parent=None, item_name="", parent_tab=None):
        ...
        self.setMinimumSize(1192, 750) # 창 너비 확장 (단위수식 +30px 추가 확장, 총 +62px)
    ...
    def set_data(self, data, current_row=-1):
        ...
        if current_row < 0 or not data:
            if hasattr(self, 'reference_popup') and self.reference_popup.model:
                print("[DEBUG] New calculation detected. Clearing reference quantities.")
    ...
    def _load_reference_codes(self):
        ...
        try:
            ...
            self.reference_codes = {str(c[0]).strip() for c in codes if c[0] is not None}
            conn.close()
        ...
        try:
            ...
            for row in rows:
                ...
                if product:
                    ...
                    # 품목 → (규격, CODE) 매핑
                    self.reference_products[product_key] = (str(spec) if spec else "", str(code) if code else "")
            
            conn.close()
            print(f"[DEBUG] Loaded {len(self.reference_products)} reference products for autocomplete.")
        except Exception as e:
    ...
    def _init_ui(self):
        ...
        self.master_table.setHorizontalHeaderLabels(["타입별 조명기구", "구분", "산출수식"])
        
        self.master_table.verticalHeader().setDefaultAlignment(Qt.AlignmentFlag.AlignCenter) # 세로 헤더 중앙 정렬
        self.master_table.verticalHeader().setFont(QFont("새굴림", 11)) # 세로 헤더 폰트 설정 (헤더는 새굴림 적용)
        self.master_table.verticalHeader().setDefaultSectionSize(22) # 행 높이 22px 설정
        self.master_table.verticalHeader().setFixedWidth(15) # [수정] 헤더 너비를 15px로 더 축소 (10% 수준으로 극소화)
        header = self.master_table.horizontalHeader()
        header.setSectionResizeMode(QHeaderView.ResizeMode.Interactive)
        
        # [NEW] 헤더 클릭 선택 기능을 위해 SelectionBehavior를 아이템 단위로 변경하고 다중 선택 허용
        self.master_table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectItems)
        self.master_table.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection)
        
        # [NEW] 헤더 클릭 시 행/열 전체 선택 연결
        self.master_table.horizontalHeader().sectionClicked.connect(self.master_table.selectColumn)
        self.master_table.verticalHeader().sectionClicked.connect(self.master_table.selectRow)
        # 다양한 입력 방식 허용 (클릭은 itemClicked에서 수동 처리)
        self.master_table.setEditTriggers(QTableWidget.EditTrigger.DoubleClicked | QTableWidget.EditTrigger.AnyKeyPressed)
        self.master_table.setFont(QFont("새굴림", 11))
        self.master_table.horizontalHeader().setFont(QFont("새굴림", 11)) # 헤더 폰트 (헤더는 새굴림 적용)
        ...
        m_palette.setColor(QPalette.ColorRole.Highlight, QColor(Qt.GlobalColor.transparent))
        m_palette.setColor(QPalette.ColorRole.HighlightedText, m_palette.color(QPalette.ColorRole.Text))
        ...
        self.detail_table.setHorizontalHeaderLabels(["W", "CODE", "산출목록", "단위수식", "계"])
        ...
        self.detail_table.verticalHeader().setDefaultSectionSize(22) # 행 높이 22px 설정
        self.detail_table.verticalHeader().setFixedWidth(15) # [수정] 헤더 너비를 15px로 더 축소 (10% 수준으로 극소화)
        
        # 헤더 폰트 및 설정
        self.detail_table.setFont(QFont("새굴림", 11))
        self.detail_table.horizontalHeader().setFont(QFont("새굴림", 11)) # 헤더 폰트 (헤더는 새굴림 적용)
        ...
        d_header.setSectionResizeMode(3, QHeaderView.ResizeMode.Stretch) # 단위수식 (여전히 자리가 남으면 확장)
        ...
        # [STYLE] 공통 스타일 적용 (무채색 통일)
        from utils.column_settings import setup_common_table
        setup_common_table(self.detail_table, ["W", "CODE", "산출목록", "단위수식", "계"])
        ...
        self.detail_table.setEditTriggers(QTableWidget.EditTrigger.DoubleClicked) # 더블 클릭 시 편집 활성화
        ...
        # [NEW] 헤더 클릭 시 행/열 전체 선택 연결
        self.detail_table.horizontalHeader().sectionClicked.connect(self.detail_table.selectColumn)
        self.detail_table.verticalHeader().sectionClicked.connect(self.detail_table.selectRow)
        ...
        for w in [self.master_table, self.detail_table, 
                  self.master_table.viewport(), self.detail_table.viewport(), self]:
    ...
    def _load_master_data(self):
        ...
        try:
            ...
            table_name = "조명기구목록"
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND (name='조명기구목록' OR name='조명기구목골');")
            ...
            if total_width > left_width_target:
                self.splitter.setSizes([left_width_target, total_width - left_width_target])
    ...
    def _on_detail_item_changed(self, item):
        ...
        if col == 3:
            ...
            self.detail_table.item(row, 4).setTextAlignment(Qt.AlignmentFlag.AlignCenter)
    ...
    def _validate_all_inputs(self):
        ...
        for row in range(1, self.detail_table.rowCount()): # [FIX] 첫 행(기구명 헤더) 제외
            # 1. W 마커 기반 상태 체크
    ...
    def _on_verify_match_clicked(self):
        ...
        try:
            ...
            project_mapping_path = os.path.join(project_folder, "project_data", "manual_mapping.json")
                    ...
                    if product_item and product_item.text().strip():
                        ...
                        if key_text in manual_mappings:
                            ...
                            if code_item:
                                ...
                                self.detail_table.setItem(row, 1, QTableWidgetItem(code_val))
                ...
                if not code_val and self.item_name == "전등수량(갯수)산출":
                    if product_item:
                        ...
                        if key_text in manual_mappings:
                            code_val = manual_mappings[key_text]
                            code_item.setText(code_val)
                            print(f"[DEBUG] Auto-applied manual CODE for {key_text}: {code_val}")
            ...
            for r_idx, row_data in enumerate(valid_rows):
                ...
                for c_idx, val in enumerate(display_data):
                    ...
                    if c_idx == 4: # '계' 컬럼 자동 계산
                        formula = str(display_data[3]) if display_data[3] is not None else ""
    ...
    def load_from_saved_state(self, saved_data):
        ...
        try:
            ...
            print(f"[DEBUG] State restored for '{self.item_name}' | Cached Masters: {len(self.master_details_cache)}")
    ...
    def save_to_gapji(self):
        ...
        try:
            ...
            sum_text = f"{int(final_sum)}" if final_sum == int(final_sum) else f"{final_sum:g}"
    ...
    def eventFilter(self, obj, event):
        ...
        if event.type() == QEvent.Type.KeyPress:
            ...
            if key == Qt.Key.Key_Tab:
                ...
                # 위젯 타입 판별 세분화
                is_on_master = (obj == self.master_table or (obj.isWidgetType() and self.master_table.isAncestorOf(obj)))
                is_on_detail = (obj == self.detail_table or (obj.isWidgetType() and self.detail_table.isAncestorOf(obj)))
                ...
                if is_on_detail:
                    ...
                    if col in [1, 2] and row >= 0:
                        ...
                        if self.detail_table.state() == QTableWidget.State.EditingState:
                            try:
                                editor = obj if isinstance(obj, QLineEdit) else self.focusWidget()
                                if editor and self.detail_table.viewport().isAncestorOf(editor):
                                    self.detail_table.commitData(editor)
                                    self.detail_table.closeEditor(editor, QAbstractItemDelegate.EndEditHint.NoHint)
                            except: pass
            ...
            if obj == self.master_table or obj == self.master_table.viewport():
                ...
                if is_digit and modifiers == Qt.KeyboardModifier.NoModifier and self.master_table.state() != QTableWidget.State.EditingState:
    ...
    def _copy_selection(self):
        ...
        self._internal_clipboard = PieceFileManager.extract_selected_rows(self.detail_table, cols_info)
        print(f"[DEBUG] Selection copied to internal clipboard: {len(self._internal_clipboard)} items")
    ...
    def _paste_selection(self):
        ...
        # 붙여넣기 수행
        PieceFileManager.insert_piece_data(self.detail_table, self._internal_clipboard, current_row, cols_info)
        
        # W 상태 재검증
        self._verify_all_codes()
        print(f"[DEBUG] Pasted {len(self._internal_clipboard)} items from internal clipboard")
    ...
    def _restore_previous_state(self):
        ...
        from PyQt6.QtWidgets import QMessageBox
        QMessageBox.information(self, "복구됨", f"이전 상태로 복구되었습니다. (남은 저장 상태: {len(self.undo_stack)}개)")
    ...
    def _auto_complete_product(self, row, product_text):
        ...
        if match_found:
            ...
            if code:
                ...
                self.detail_table.item(row, 1).setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self.detail_table.blockSignals(False)
    ...
    def restore_state(self):
        ...
        try:
            ...
            if "detail_table" in state:
                ...
                # 스크롤 위치 복구
                self.detail_table.verticalScrollBar().setValue(detail_state.get("scroll_v", 0))
                self.detail_table.horizontalScrollBar().setValue(detail_state.get("scroll_h", 0))
            
            # master_table 스크롤 복구
            if "master_table" in state:
                master_state = state["master_table"]
                self.master_table.verticalScrollBar().setValue(master_state.get("scroll_v", 0))
                self.master_table.horizontalScrollBar().setValue(master_state.get("scroll_h", 0))
...
class LightingPowerManager:
    ...
    def __init__(self, parent_tab):
        ...
        self.file_path = r"D:\이지맥스\사용자목록\전등,전열 산출공종.txt"

    def create_side_panel(self):
        """을지 우측에 배치될 산출공종 패널 생성"""
        self.panel = QFrame()
        self.panel.setFixedWidth(250)
        self.panel.setStyleSheet("background-color: #f8f9fa; border-left: 1px solid #dee2e6;")
        ...
        header.setStyleSheet("background-color: #e9ecef; color: #495057; font-family: '새굴림'; font-size: 11pt;")
    ...
    def _process_selection_and_open(self, title):
        ...
        table.setItem(current_row, self.parent_tab.EULJI_COLS["ITEM"], QTableWidgetItem(title))
        ...
        try:
            ...
            if popup.exec() == QDialog.DialogCode.Accepted:
                ...
                if save_payload:
                    ...
                    table.setItem(current_row, self.parent_tab.EULJI_COLS["ITEM"], QTableWidgetItem("조명기구 Type산출"))
                    
                    # [Step 12] 페이로드에서 합계 금액 가져오기
                    total_sum = save_payload.get("final_total", 0.0)
                    
                    # 2. 산출수식 (FORMULA) -> 총 합계값
                    sum_text = f"{int(total_sum)}" if total_sum == int(total_sum) else f"{total_sum:g}"
                    table.setItem(current_row, self.parent_tab.EULJI_COLS["FORMULA"], QTableWidgetItem(sum_text))
                    
                    # 3. 단위 (UNIT) -> "식"
                    unit_item = QTableWidgetItem("식")
                    unit_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                    table.setItem(current_row, self.parent_tab.EULJI_COLS["UNIT"], unit_item)
                    
                    # 4. 계 (TOTAL) -> "1"
                    qty_item = QTableWidgetItem("1")
                    qty_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                    table.setItem(current_row, self.parent_tab.EULJI_COLS["TOTAL"], qty_item)
                    
                    # [Step 12] 상세 데이터 전체를 UserRole에 영구 저장 (재진입용 핵심)
                    item_widget = table.item(current_row, self.parent_tab.EULJI_COLS["ITEM"])
                    item_widget.setData(Qt.ItemDataRole.UserRole, save_payload)
                    ...
                    if hasattr(self.parent_tab, 'current_gongjong') and self.parent_tab.current_gongjong:
                        self.parent_tab._save_eulji_data(self.parent_tab.current_gongjong)
                        self.parent_tab._update_gapji_marker(self.parent_tab.current_gongjong)
                        
                    cached_count = len(save_payload.get("master_details_cache", {}))
                    print(f"[DEBUG] Applied single row with {cached_count} master types.")
        finally:
