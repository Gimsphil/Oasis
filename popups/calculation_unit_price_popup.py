...
class CalculationUnitPricePopup(QDialog):
    ...
    def _init_ui(self):
        ...
        self.lbl_item_name.setStyleSheet(
            "font-family: '굴림체'; font-size: 10pt; font-weight: bold;"
        )
        ...
        self.table.setItemDelegateForColumn(
            UNIT_PRICE_COLS["MARK"],
            CenterAlignmentDelegate(self.table, [UNIT_PRICE_COLS["MARK"]]),
        )
    ...
    def _create_bottom_menu(self, parent_layout):
        ...
        c2, self.btn_unitprice = make_btn(
            "일위대가", "F11", self._on_unitprice_click, 75
        )
        ...
        c7, self.btn_list_switch = make_btn(
            "작업목록바꾸기", "PgUp/Dn", self._on_list_switch_click, 110
        )
        c8, self.btn_screen_copy = make_btn(
            "현재화면복사", "F9", self._on_screen_copy_click, 100
        )
    ...
    def _on_block_click(self):
        ...
        if not self._is_block_mode:
            ...
            sel_range = QTableWidgetSelectionRange(
                start, 0, end, self.table.columnCount() - 1
            )
    ...
    def _on_piece_save_click(self):
        ...
        if PieceFileManager.save_piece_file(self, data):
            QMessageBox.information(
                self, "조각파일저장", f"{len(data)}개 행이 조각파일로 저장되었습니다."
            )
    ...
    def _on_piece_load_click(self):
        ...
        if not data:
            QMessageBox.warning(
                self, "조각파일", "파일을 불러올 수 없거나 데이터가 비어있습니다."
            )
            return
        ...
        if event.type() in [QEvent.Type.KeyPress, QEvent.Type.ShortcutOverride]:
            ...
            if (
                ...
                if key == Qt.Key.Key_F5:
                    ...
                    self._on_screen_copy_click()
                    return True
                elif key == Qt.Key.Key_F10:
                    self._on_paste_click()
                    return True
                elif key == Qt.Key.Key_F11:
                    self._on_unitprice_click()
                    return True
            ...
            if key == Qt.Key.Key_Tab and modifiers == Qt.KeyboardModifier.NoModifier:
                ...
                is_target_obj = (
                    ...
                    or (
                        hasattr(self.table, "viewport") and obj == self.table.viewport()
                    )
                )
    ...
    def _on_table_list_pick(self, row, col):
        ...
        if self.parent_tab:
            print(
                f"[DEBUG] parent_tab has reference_popup? {hasattr(self.parent_tab, 'reference_popup')}"
            )
        ...
        if self.parent_tab and hasattr(self.parent_tab, "reference_popup"):
            ...
            if self.parent_tab.reference_popup is None:
                ...
                self.parent_tab.reference_popup = DatabaseReferencePopup(
                    self.parent_tab
                )

            print("[DEBUG] Opening reference popup...")
            self.hide()  # 자료사전이 보이도록 일시 숨김
            self.parent_tab.reference_popup.parent_popup = (
                self  # 일시적으로 부모를 이 팝업으로 설정
            )
            ...
            print(
                "[ERROR] Cannot open reference popup: parent_tab is missing or has no reference_popup"
            )
    ...
    def _on_list_button_clicked(self):
        ...
        try:
            ...
            if ref_popup.exec():
                ...
                item = self.parent_tab.eulji_table.item(
            ...
            error_msg = (
                f"[ERROR] _on_list_button_clicked failed: {e}\n{traceback.format_exc()}"
            )
            print(error_msg)
            try:
    ...
    def _load_data(self, item_name):
        ...
        try:
            ...
            log_msg = (
                f"[{datetime.now()}] _load_data for '{item_name}' (Path: {file_path})\n"
            )
                ...
                try:
                    ...
                    if isinstance(data, list):
                        for row_data in data:
                            ...
                            self.table.setItem(
                                row, self.UNIT_PRICE_COLS["MARK"], mark_item
                            )

                            self.table.setItem(
                                row,
                                self.UNIT_PRICE_COLS["LIST"],
                                QTableWidgetItem(str(row_data.get("list", ""))),
                            )
                            self.table.setItem(
                                row,
            ...
            if not loaded:
                for i in range(30):
                    ...
                    if i == 0 and item_name:
                        self.table.setItem(
                            0, self.UNIT_PRICE_COLS["LIST"], QTableWidgetItem(item_name)
                        )
        ...
        try:
            ...
            for r in range(self.table.rowCount()):
                ...
                if mark_text or list_text or qty_text:
                    data.append({"mark": mark_text, "list": list_text, "qty": qty_text})
                    has_valid_data = True
    ...
    def _evaluate_math(self, expression):
        """사칙연산 수식을 계산하여 결과를 반환 (새로운 파서 사용)"""
        if not expression:
            return 0.0
        try:
            # 전처리: x/X를 *로 변경 (단위기호)
    ...
    def _on_unit_price_cell_changed(self, row, col):
        ...
        if valid_count > self.MAX_UNIT_PRICE_ROWS:
            print(
                f"[WARN] Unit price rows ({valid_count}) exceed max ({self.MAX_UNIT_PRICE_ROWS})"
            )
    ...
    def _sync_to_parent(self):
        ...
        try:
            ...
            if has_data:
                ...
                e_table.setItem(
                    self.target_row, formula_col, QTableWidgetItem(new_formula)
                )
                ...
                if not existing_text and first_row_text:
                    e_table.setItem(
                        self.target_row, item_col, QTableWidgetItem(first_row_text)
                    )
                ...
                print(
                    f"[DEBUG] Parent row {self.target_row} synced. Multiplier: {total_multiplier}"
                )
        except Exception as e:
    ...
    def _adjust_position(self):
        ...
        if self.parent_tab and hasattr(self.parent_tab, "eulji_table"):
            ...
            print(
                f"[DEBUG] Popup moved to: ({target_x}, {target_y}) - right of table viewport"
            )
    ...
    def prepare_show(self, parent_tab, row, col):
        ...
        try:
            ...
            if parent_tab and hasattr(parent_tab, "eulji_table"):
                ...
                print(
                    f"[DEBUG] formula parsed: base='{self.base_formula}', multiplier={self.current_multiplier}"
                )
            ...
            if this_old_row == row and not prev_label and item_name:
                print(
                    f"[DEBUG] Row {row} now has a name: '{item_name}'. Syncing unnamed data."
                )
            ...
            self.lbl_item_name.setText(
                f"산출일위목록: {item_name if item_name else '-'}"
            )
            ...
            try:
                with open(self.debug_log, "a", encoding="utf-8") as f:
                    f.write(
                        f"[{datetime.now()}] prepare_show: row={row}, col={col}, item='{item_name}'\n"
                    )
            except:
                pass
    ...
    def show_popup(self):
        ...
        try:
            ...
            try:
                with open(self.debug_log, "a", encoding="utf-8") as f:
                    f.write(
                        f"[{datetime.now()}] show_popup finished. Visible: {self.isVisible()}\n"
                    )
            except:
                pass
        except Exception as e:
...
class UnitPriceDelegate(CleanStyleDelegate):
    ...
    def createEditor(self, parent, option, index):
        print(
            f"[DEBUG] UnitPriceDelegate.createEditor called for row={index.row()}, col={index.column()}"
        )
        editor = super().createEditor(parent, option, index)
        if isinstance(editor, QLineEdit) and self.popup:
            print(
                f"[DEBUG] Installing event filter on editor for row={index.row()}, col={index.column()}"
            )
